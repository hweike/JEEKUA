...
interface HeadingTextFieldProps {
  field: any;
...
export function HeadingTextField({ field, value, onChange, readOnly }: HeadingTextFieldProps) {
  ...
  useEffect(() => {
    if (isFirstRender.current && (!value || typeof value !== 'object' || !value.textId)) {
      const newValue = { zh: '', en: '', textId: nanoid() };
      onChange(newValue);
    }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <label className="text-sm font-medium text-foreground">{field.label || '标题文本'}</label>
        <select
          value={currentLocale}
      ...
      <input
        ...
        placeholder={`输入标题 (${currentLocale === 'zh' ? '中文' : '英文'})`}
        disabled={readOnly}
      />
    </div>
  );
