// components/admin/analytics/PerformanceEnvironment.tsx
...
interface PerformanceEnvironmentProps {
  data: EnvironmentPerformance[];
  metricKey: string;
  metricLabel: string;
  loading?: boolean;
}
...
export function PerformanceEnvironment({
  ...
  return (
    <div className="bg-white border border-edge rounded-lg shadow-sm p-4 md:p-6">
      <h2 className="text-lg font-semibold text-gray-800 mb-3">环境</h2>
      ...
      <div className="flex justify-between items-center px-2 py-1 text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-100">
        <span>{activeTab === 'device' ? '设备' : '浏览器'}</span>
      ...
      {displayData.length === 0 ? (
        ...
        <div className="divide-y divide-gray-100">
          {displayData.map((item, index) => {
            const value = item[metricKey as keyof EnvironmentPerformance] as number || 0;
            return (
