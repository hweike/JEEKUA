...
import { downloadAndSaveImage } from '@/lib/files/download'; // 公共函数
...
// ========== 获取属性模板（使用请求 origin 修复相对路径问题） ==========
let cachedAttributeTemplates: any[] = [];
let cacheTimestamp = 0;
const CACHE_TTL = 60 * 1000;

async function getAttributeTemplates(locale: string, requestOrigin: string): Promise<any[]> {
  const now = Date.now();
  if (cachedAttributeTemplates.length > 0 && now - cacheTimestamp < CACHE_TTL) {
    return cachedAttributeTemplates;
  }
  try {
    // 使用请求的 origin 构建完整 URL，确保服务端 fetch 能正确访问
...
export async function POST(request: NextRequest) {
  try {
    ...
    const attributeTemplates = await getAttributeTemplates(locale, requestOrigin);
    ...
    async function processCategory2(
      ...
      if (existingSeries) {
        ...
        if (coverImageUrl === '') {
          ...
          imageTasks.push({
            target: existingSeries,
            field: 'image',
            url: coverImageUrl,
            referenceType: 'series',
            referenceId: existingSeries.id,
          });
        } else {
    ...
    if (imageTasks.length > 0) {
      await Promise.all(imageTasks.map(async (task) => {
        try {
          const savedUrl = await downloadAndSaveImage(task.url, {
            referenceType: task.referenceType,
            referenceId: task.referenceId,
          });
          task.target[task.field] = savedUrl;
        } catch (err) {
          console.error(`下载图片失败: ${task.url}`, err);
          task.target[task.field] = task.url; // 降级保留原 URL
        }
      }));
    }
