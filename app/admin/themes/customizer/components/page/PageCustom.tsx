// app/admin/themes/customizer/components/page/PageCustom.tsx
'use client';
...
export default function PageCustom({ theme, globalTheme, onUpdate }: PageCustomProps) {
  const { colors } = theme;
  ...
  const getEffectiveGlobalValue = (key: string): string => {
    ...
    const config = getPageVariableConfig('custom', key);
  ...
  // 获取普通页面的分组配置
  const groups = getPageVariableGroups('custom');
