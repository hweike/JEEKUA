...
export default function PageProducts({ theme, globalTheme, onUpdate }: PageProductsProps) {
  const { colors } = theme;
  ...
  const getEffectiveGlobalValue = (key: string): string => {
    ...
    const config = getPageVariableConfig('products', key);
  ...
  // 获取产品页的分组配置
  const groups = getPageVariableGroups('products');
