// app/admin/themes/customizer/components/page/PageAccount.tsx
'use client';
...
export default function PageAccount({ theme, globalTheme, onUpdate }: PageAccountProps) {
  const { colors } = theme;
  ...
  const getEffectiveGlobalValue = (key: string): string => {
    ...
    const config = getPageVariableConfig('account', key);
  ...
  // 获取用户中心页的分组配置
  const groups = getPageVariableGroups('account');
