// app/admin/themes/customizer/components/page/PageSearch.tsx
'use client';
...
export default function PageSearch({ theme, globalTheme, onUpdate }: PageSearchProps) {
  const { colors } = theme;
  ...
  const getEffectiveGlobalValue = (key: string): string => {
    ...
    const config = getPageVariableConfig('search', key);
  ...
  // 获取搜索页的分组配置
  const groups = getPageVariableGroups('search');
