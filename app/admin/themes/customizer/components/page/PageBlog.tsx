...
export default function PageBlog({ theme, globalTheme, onUpdate }: PageBlogProps) {
  const { colors } = theme;
  ...
  const getEffectiveGlobalValue = (key: string): string => {
    ...
    const config = getPageVariableConfig('blog', key);
  ...
  // 获取博客页的分组配置
  const groups = getPageVariableGroups('blog');
