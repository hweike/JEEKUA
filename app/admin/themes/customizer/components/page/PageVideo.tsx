// app/admin/themes/customizer/components/page/PageVideo.tsx
'use client';
...
export default function PageVideo({ theme, globalTheme, onUpdate }: PageVideoProps) {
  const { colors } = theme;
  ...
  const getEffectiveGlobalValue = (key: string): string => {
    ...
    const config = getPageVariableConfig('video', key);
  ...
  // 获取视频页的分组配置
  const groups = getPageVariableGroups('video');
