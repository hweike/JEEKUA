...
export default function PageDocs({ theme, globalTheme, onUpdate }: PageDocsProps) {
  const { colors } = theme;
  ...
  const getEffectiveGlobalValue = (key: string): string => {
    ...
    const config = getPageVariableConfig('docs', key);
  ...
  // 获取文档页的分组配置
  const groups = getPageVariableGroups('docs');
