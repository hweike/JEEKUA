...
export default function PageHome({ theme, globalTheme, onUpdate }: PageHomeProps) {
  const { colors } = theme;
  ...
  const getEffectiveGlobalValue = (key: string): string => {
    ...
    const config = getPageVariableConfig('home', key);
  ...
  // 获取首页的分组配置
  const groups = getPageVariableGroups('home');
