...
/**
 * 批量获取产品 slug（带内存缓存，直接使用 Supabase）
 */
async function getProductSlugs(productIds: string[]): Promise<Record<string, string>> {
  ...
  if (uncachedIds.length > 0) {
    try {
      ...
      if (!error && data) {
        data.forEach((item: { productId: string; slug: string | null }) => {
          if (item.productId) {
            const slug = item.slug || '';
            result[item.productId] = slug;
            slugCache.set(item.productId, { slug, timestamp: now });
          }
        });
      } else {
...
export default function OrdersPage() {
  ...
  useEffect(() => {
    ...
    const loadUsers = async () => {
      ...
      if (cachedUsers && (now - cachedUsersTimestamp) < CACHE_TTL) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const userList = (data?.users ?? data?.data ?? data ?? []) as Array<{ id: string; name?: string; email?: string }>;
        ...
        if (productIds.length > 0) {
          ...
          if (needFetchIds.length > 0) {
            ...
            if (idsToLoad.length > 0) {
              ...
              const loadSlugs = () => {
                getProductSlugs(idsToLoad).then(newSlugs => {
  ...
  // ============================================================
  const copyOrderInfo = useCallback(async (orderNo: string, contractNo?: string) => {
  ...
  const handleConfirmPayment = useCallback(async (data: { depositAmount: number; depositPercent: number }) => {
    if (!selectedOrderId) return;
    
    setModalLoading(true);
    try {
      const res = await fetch(`/api/admin/payment/orders/${selectedOrderId}/actions?action=confirm_payment`, {
        method: 'POST',
  ...
  // ✅ 确认发货 - 弹窗（支持两种模式）
  // ============================================================
  const openShippingModal = useCallback((orderId: string, mode: 'confirm' | 'management' = 'confirm') => {
    const order = getOrderById(orderId);
    if (order) {
      setSelectedOrderId(orderId);
      setSelectedOrder(order);
  ...
  // ✅ 确认发货（paid → completed）
  // ============================================================
  const handleConfirmShipping = useCallback(async (data: {
    ...
    setModalLoading(true);
    try {
      const res = await fetch(`/api/admin/payment/orders/${selectedOrderId}/actions?action=confirm_shipping`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  // ============================================================
  const handleSaveShippingRecords = useCallback(async (records: ShippingRecord[]) => {
    if (!selectedOrderId) return;
    
    setModalLoading(true);
    try {
      const res = await fetch(`/api/admin/payment/orders/${selectedOrderId}/shipping-records`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSubmit = useCallback(async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/payment/orders/${id}/actions?action=submit`, {
        method: 'POST',
      });
      const data = await res.json();
      if (data.success && data.data?.shareUrl) {
        await copyToClipboard(data.data.shareUrl);
  ...
  const handleRecall = useCallback(async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/payment/orders/${id}/actions?action=recall`, {
        method: 'POST',
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleCancel = useCallback(async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/payment/orders/${id}/actions?action=cancel`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleReorder = useCallback(async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/payment/orders/${id}/actions?action=reorder`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
  ...
  const handleDuplicate = useCallback(async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/payment/orders/${id}/actions?action=duplicate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          operator: currentUser?.id || currentUser?.name || 'system' 
  ...
  const handleCopyLink = useCallback(async (id: string) => {
    try {
      const res = await fetch(`/api/admin/payment/orders/${id}/actions?action=copy_link`, {
        method: 'POST',
      });
      const data = await res.json();
      if (data.success && data.data?.shareUrl) {
  ...
  const handleAction = useCallback(async (action: string, orderId: string) => {
    ...
    try {
      ...
      if (!['view', 'edit', 'confirm_payment', 'confirm_shipping', 'shipping_management'].includes(action)) {
        setActionLoading(null);
      }
    }
  }, [
