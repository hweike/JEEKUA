import { getCustomerById } from '@/lib/CRM/storage';
...
export default async function CustomerDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <Link href="/admin/crm" className="inline-flex items-center gap-1 text-blue-600 mb-4"><ArrowLeft size={16} /> 返回列表</Link>
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div><span className="font-medium">官网:</span> <a href={customer.website} target="_blank" className="text-blue-500">{customer.website}</a></div>
          <div><span className="font-medium">地址:</span> {customer.address}</div>
          <div><span className="font-medium">阶段:</span> {customer.stage}</div>
          <div><span className="font-medium">重要等级:</span> <div className="inline-flex gap-0.5">{Array(3).fill(0).map((_,i) => <Star key={i} size={14} className={i < customer.importance ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'} />)}</div></div>
