...
import FooterForm from '@/components/SiteHeadersFooters/FooterForm';
import { FooterConfig } from '@/lib/SiteHeadersFooters/types';
  ...
  const initConfig = async (lang: string) => {
    try {
      const res = await fetch('/api/SiteHeadersFooters/init', {
        ...
        body: JSON.stringify({ type: 'footer', locale: lang }),
      });
      if (!res.ok) throw new Error('初始化失败');
      setToast({ message: `${lang} 页脚配置初始化成功`, type: 'success' });
      await loadConfig(lang);
    } catch (error) {
      setToast({ message: `${lang} 页脚配置初始化失败`, type: 'error' });
    }
  };

  const handleInitCurrent = async () => {
    if (confirm(`确定要将 ${locale} 语言的页脚设置恢复为样本默认值吗？`)) {
      ...
      <div className="min-h-screen bg-gray-100 py-8">
        <div className="w-4/5 mx-auto bg-white rounded-lg shadow p-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold">页脚设置 - {locale.toUpperCase()}</h1>
            <LanguageSelector currentLocale={locale} onLocaleChange={setLocale} displayMode="zh" />
          </div>
          <div className="text-center py-12">
            <p className="text-gray-500 mb-4">当前语言尚无页脚配置数据</p>
            <button
    ...
    <div className="min-h-screen bg-gray-100 py-8">
      <div className="w-4/5 mx-auto">
        ...
        <div className="bg-white rounded-lg shadow p-[50px]">
          <FooterForm initialConfig={config} locale={locale} />
        </div>
      </div>
