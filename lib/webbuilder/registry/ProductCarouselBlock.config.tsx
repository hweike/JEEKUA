...
import type { ProductCarouselBlockProps } from '@/lib/webbuilder/types';
...
export const config: ComponentConfig<ProductCarouselBlockProps> = {
  label: '精选产品轮播',
  category: 'Product',
  defaultProps: {
  ...
  fields: {
    ...
    carouselGroup: {
      ...
      objectFields: {
        ...
        gap: { label: '卡片间距 (px)', type: 'number', min: 0, max: 60, step: 4 },
        loop: {
          label: '循环',
          type: 'radio',
          options: [
  ...
  render: ({ puck, ...props }) => (
    <ProductCarouselBlock puck={puck} {...(props as ProductCarouselBlockProps)} />
  ),
};